...
class SavedScreenState extends State<SavedScreen> {
  ...
  bool _bannerReady = true;
  bool _nativeReady = true;

  // Analytics label + dedicated cache slot for the Saved screen native ad.
  static const String _nativePlacement = 'GTL_native_saved_screen';
  static const String _nativeCacheKey = 'saved_screen';
  ...
  void initState() {
    ...
    HomeBottomNativeAd.preload(
      key: _nativeCacheKey,
      adUnitId: AdIds.savedScreenNative,
    );
    _loadSavedItems();
  }
  ...
  Widget _buildBody(AppStrings t) {
    // The native ad shows regardless of whether the list has items.
    final Widget? nativeAd = _nativeReady ? _buildNativeAd() : null;

    if (_savedItems.isEmpty) {
      // Native ad on top, the empty-state message below it.
      return Column(
        children: [
          if (nativeAd != null)
            Padding(
          ...
          Expanded(child: Center(child: _emptyState(t))),
        ],
      );
    }

    // Ordered rows: saved items, with the banner inserted at position 2
    // (index 1) and the native ad at position 3 (index 2). insert() clamps
    // to the list length so short lists simply append the ads.
    final rows = <Widget>[
      for (var i = 0; i < _savedItems.length; i++) _buildItemCard(t, i),
    ];
    if (_bannerReady) {
      // The list is non-empty here, so index 1 is always valid.
      rows.insert(1, _buildBanner());
    }
    if (nativeAd != null) {
      rows.insert(rows.length < 2 ? rows.length : 2, nativeAd);
    }
  ...
  Widget _buildNativeAd() {
    return HomeBottomNativeAd(
      adUnitId: AdIds.savedScreenNative,
      placement: _nativePlacement,
      cacheKey: _nativeCacheKey,
      onLoadFailed: (_, _) {
  ...
  Widget _buildBanner() {
    return Center(
      child: GlobalBannerAd(
        ...
        onAdReady: (ready) {
          if (!ready && mounted) setState(() => _bannerReady = false);
        },
      ),
    );
  }

  Widget _emptyState(AppStrings t) {
    return Column(
  ...
  Widget _buildItemCard(AppStrings t, int itemIndex) {
